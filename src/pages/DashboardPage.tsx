...
export default function DashboardPage() {
  const { user, assessments } = useAppStore();
  ...
  const avgExam = attempts ? Math.round(assessments.reduce((s, a) => s + a.examScore, 0) / attempts) : 0;
  const avgTime = attempts ? (assessments.reduce((s, a) => s + a.timeSpent, 0) / attempts).toFixed(1) : '0';
  ...
  const courseBreakdown = assessments.reduce((acc, a) => {
    ...
    acc[a.course].examSum += a.examScore;
    return acc;
  }, {} as Record<string, { total: number; quizSum: number; examSum: number }>);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container max-w-6xl py-8">
        <div className="animate-fade-in rounded-2xl border bg-card p-6 sm:p-8 shadow-soft">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Your active learning record and progress at a glance.</p>
            </div>
        ...
        <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-4 stagger-children">
          ...
          <MetricCard icon={TrendingUp} label="Avg Exam" value={avgExam} subtitle="Out of 100" />
                ...
                {[
                  ...
                  <div key={label} className="flex justify-between items-center py-1.5 border-b border-border/50 last:border-0">
                    ...
                    <span className={cn('font-medium',
                      label === 'Proficiency' && value === 'Advanced' && 'text-success',
                      label === 'Proficiency' && value === 'Intermediate' && 'text-warning',
                      label === 'Proficiency' && value === 'Beginner' && 'text-primary',
                    )}>{value}</span>
                  </div>
                ))}
                <div className="pt-2">
            ...
            {Object.keys(courseBreakdown).length > 0 ? (
              <div className="space-y-5">
                {Object.entries(courseBreakdown).map(([course, data]) => {
                  const avgScore = Math.round((data.quizSum + data.examSum) / (data.total * 2));
                  return (
                    <div key={course}>
        ...
        <div className="mt-6 grid sm:grid-cols-3 gap-4 stagger-children">
          ...
          <div className="rounded-xl border bg-card p-5 shadow-soft text-center">
            <ProgressRing value={avgExam} size={72} strokeWidth={6} color="hsl(var(--success))" label="Exam Avg" />
          </div>
          <div className="rounded-xl border bg-card p-5 shadow-soft text-center">
            <div className="flex flex-col items-center gap-1">
              <Clock className="h-6 w-6 text-muted-foreground" />
              <span className="text-lg font-bold mt-2">{avgTime}h</span>
